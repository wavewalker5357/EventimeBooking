...
class Registration
{
    ...
    private function registerNewUser()
    {
        if (empty($_POST['user_name'])) {
            ...
            $this->errors[] = "Username does not fit the name scheme: only a-Z and numbers are allowed, 2 to 30 characters";
            ...
            $this->errors[] = "Home address cannot be longer than 200 characters";
        } elseif (strlen($_POST['user_mobile_telephone']) > 15) {
            $this->errors[] = "Mobile number cannot be longer than 15 digits";
           ...
           $this->errors[] = "Birth date does not fit the date scheme: yyyy-mm-dd (for ex.: 2001-01-01) or are not between 1900-01-01 and 2099-12-31";
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                $user_home_address = $this->db_connection->real_escape_string(strip_tags($_POST['user_home_address'], ENT_QUOTES));
                $user_mobile_telephone = $this->db_connection->real_escape_string(strip_tags($_POST['user_mobile_telephone'], ENT_QUOTES));
                $user_telephone = $this->db_connection->real_escape_string(strip_tags($_POST['user_telephone'], ENT_QUOTES));
                $user_birth = $this->db_connection->real_escape_string(strip_tags($_POST['user_birth'], ENT_QUOTES));
